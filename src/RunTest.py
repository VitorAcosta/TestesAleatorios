...
class RunTest():
  ...
  def monobit_test(self, key, verbose = False):
    ...
    if 9654 < count < 10346:
      return (True, count) if verbose else True
    else:
      return (False, count) if verbose else False


  def poker_test(self, key, verbose = False):
    nibbles_count = np.zeros(16)
    for x in range(int(len(key)/4)):
      nibble_idx = int(key[x*4:(x+1)*4], 2)
      nibbles_count[nibble_idx] += 1

    x = 16.0/float(len(key)/4) * sum(nibbles_count**2) - (len(key)/4)

    verbose_dict = dict()
    for idx,i in enumerate(nibbles_count):
      verbose_dict[idx] = i

    if 1.03 < x < 57.4:
      return (True, verbose_dict, round(x,3)) if verbose else True
    return (False, verbose_dict, round(x,3)) if verbose else False
